...
public class SystematicRipperStarter implements RipperEventListener {
	...
	*/
	...
	public SystematicRipperStarter(String configFile) {
		super();
	...
	public void startRipping()
	{
		...
		if (conf != null)
		{
			...
			SystematicDriver.PULL_COVERAGE = pullCoverage.equals("1");
			SystematicDriver.PULL_COVERAGE_ZERO = pullCoverageZero.equals("1");
			SystematicDriver.COVERAGE_PATH = coveragePath;
			SystematicDriver.REPORT_FILE = reportFile;
			SystematicDriver.LOG_FILE_PREFIX = logFilePrefix;
			SystematicDriver.AVD_NAME = avd_name;
			SystematicDriver.EMULATOR_PORT = Integer.parseInt(avd_port);
			SystematicDriver.AUT_PACKAGE = aut_package;
			SystematicDriver.AUT_MAIN_ACTIVITY = aut_main_activity;
			
			SystematicDriver.SLEEP_AFTER_TASK = Integer.parseInt(sleep_after_task);
			
			SystematicDriver.PING_MAX_RETRY = Integer.parseInt(ping_max_retry);
			SystematicDriver.ACK_MAX_RETRY =  Integer.parseInt(ack_max_retry);
			SystematicDriver.FAILURE_THRESHOLD = Integer.parseInt(failure_threshold);
			SystematicDriver.PING_FAILURE_THRESHOLD = Integer.parseInt(ping_failure_threshold);
			...
			SystematicDriver.LOGCAT_PATH = logcatPath;
			SystematicDriver.XML_OUTPUT_PATH = xmlOutputPath;
			SystematicDriver.JUNIT_OUTPUT_PATH = junitOutputPath;
